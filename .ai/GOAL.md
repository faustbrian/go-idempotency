...
- PostgreSQL adapter using atomic statements, row locking where needed, and
  migrations through `go-migrations`.
...
- `go-webhook` integration for provider delivery deduplication.
- `go-queue` middleware for consumer deduplication and handler ownership.
- `go-outbox` integration for transactionally related records where appropriate.
- Command/import helpers for named operations and source-record identities.
- Optional `go-log` and `go-telemetry` integration with key hashing and bounded
  cardinality.
