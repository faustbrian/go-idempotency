...
`go-idempotency` provides durable ownership, fencing, and bounded result replay
...
and import helpers, and bounded logging and telemetry observers are implemented.
The API remains pre-v1 and is not yet released as stable.
...
The [transaction and outbox guide](docs/outbox.md) shows atomic business and
completion commits with `CompleteTx`.
...
retention, cleanup, capacity, and incident recovery. See
