...
The memory adapter uses its injected clock for deterministic tests. Durable
adapters use backend-authoritative time for ownership comparisons so process
clock skew cannot let a caller extend or complete a lease. Returned timestamps
are diagnostic evidence; clients must not use local time to override a backend
decision.
