...
`go-log` and `go-telemetry`, so they do not require global logger or telemetry
registration:
...
`idempotencylog.New` accepts the `*slog.Logger` returned by `go-log`.
`idempotencytelemetry.New` accepts the standard OpenTelemetry meter provider
returned by `go-telemetry.Runtime.MeterProvider`. Both upstream modules are
currently untagged, so this module binds their explicit standard contracts
rather than pinning an unreleased source commit. Compile the adapters against
the upstream revision selected by the application as part of its upgrade test.
