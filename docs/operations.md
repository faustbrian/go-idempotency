...
For PostgreSQL, apply `postgres.SchemaMigration()` before the application is
ready. Verify connectivity, table permissions, the intended `search_path`, and
pool capacity. Do not run schema creation in request startup.
...
deletes every record and separate processes elect separate owners.
...
Use `idempotencylog` with the standard logger returned by `go-log`, and use
`idempotencytelemetry` with `go-telemetry.Runtime.MeterProvider()`. The metric
