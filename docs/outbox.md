...
`outboxWriter` is a `*go-outbox/postgres.Writer`, and `envelope` is a bounded
`go-outbox.Envelope`. `InsertAndComplete` calls `Writer.Insert` before
...
The root module does not import `go-outbox`. A pinned Go 1.26 compatibility
module proves that `go-outbox/postgres.Writer` satisfies the generic writer
