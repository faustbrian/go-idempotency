...
Apply `postgres.SchemaMigration().Up` with your deployment migration system,
then construct a store from the application's pgx pool:
...
`create_idempotency_records`, and `Up` and `Down` SQL fields. The required
`go-migrations` repository currently exposes no module or registration API, so
the direct binding cannot be implemented without inventing an incompatible
contract. Keep migration application in deployment, not request startup; the
descriptor is ready to bind once that API is published.
