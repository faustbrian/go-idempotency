...
an immutable `go-migrations.Migration`, including its default transactional
mode and content checksum. Add that value to the `go-migrations` source used by
...
`go-outbox/postgres.Writer` and the idempotency PostgreSQL store through their
exact transaction method contracts.

The isolated `compatibility/ecosystem` module pins and compiles the current
`go-log`, `go-migrations`, `go-outbox`, `go-queue`, `go-telemetry`, and
`go-webhook` revisions under Go 1.26. It proves the exact public interfaces and
