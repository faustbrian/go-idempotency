...
## `go-webhook` replay store

`go-webhook/adapters/goidempotency` implements its exact `webhook.ReplayStore`
...
Pass `replayStore` to the `go-webhook` verifier. Its atomic first check returns
recorded, a retained matching check returns replayed, and storage failure fails
closed. The expiry supplied by `go-webhook` becomes the ownership lease, so a
