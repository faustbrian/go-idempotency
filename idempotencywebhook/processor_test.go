...
import (
	...
	"github.com/faustbrian/go-idempotency/idempotencywebhook"
...
func TestProcessorPreservesHandlerFailureAndTypedWrapper(t *testing.T) {
	...
	var typed func(context.Context, providerDelivery) error = wrapped
	if err := typed(context.Background(), delivery); !errors.Is(err, handlerErr) {
